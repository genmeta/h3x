...
async fn request_accept_and_respond() {
    ...
    let (response, served) =
        tokio::join!(client::request(request, client_recv, client_send), async {
            let request = server::accept(server_recv).await?;
            ...
            server::respond(response, server_send).await
        });
...
async fn streaming_echo() {
    ...
    let (response, uploaded, served) = tokio::join!(
        client::request(request, client_recv, client_send),
        async {
        ...
        async {
            ...
            let (sent, echoed) = tokio::join!(server::respond(response, server_send), async {
                let mut buf = [0; 3];
                loop {
