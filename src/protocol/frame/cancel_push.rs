    ...
    pub(crate) async fn be_frame<T: AsyncRead + Unpin + ?Sized>(
        ...
        if length.into_u64() > VarInt::MAX_SIZE as u64 {
            return Err(ErrorCode::H3_FRAME_ERROR
                .with_reason("CANCEL_PUSH payload exceeds the maximum identifier size"));
        }
        ...
        let id = be_varint(&mut payload)
            ...
            .ok_or_else(|| {
                ErrorCode::H3_FRAME_ERROR
                    .with_reason("CANCEL_PUSH payload is missing a complete identifier")
            })?;
        if payload.limit() != 0 {
            return Err(
                ErrorCode::H3_FRAME_ERROR.with_reason("CANCEL_PUSH payload has trailing bytes")
            );
        }
        Ok(Self {
            length,
