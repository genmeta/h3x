...
pub(super) async fn be_string_literal<T: AsyncRead + Unpin + ?Sized>(
    ...
    validate_prefix_bits(prefix_bits).map_err(|_| ErrorCode::QPACK_ENCODER_STREAM_ERROR)?;
...
pub(super) async fn be_string_literal_with_first<T: AsyncRead + Unpin + ?Sized>(
    ...
    let value = if first & (1 << (prefix_bits - 1)) != 0 {
        decode_huffman(&encoded).map_err(|_| ErrorCode::QPACK_ENCODER_STREAM_ERROR)?
    } else {
        Bytes::from(encoded)
    };
...
pub(super) fn be_string_literal_slice(input: &[u8], prefix_bits: u8) -> Result<(&[u8], Bytes)> {
    ...
    let length = usize::try_from(length).map_err(|_| ErrorCode::QPACK_DECOMPRESSION_FAILED)?;
...
fn decode_huffman(encoded: &[u8]) -> Result<Bytes> {
    ...
    httlib_huffman::decode(encoded, &mut decoded, DecoderSpeed::FourBits)
        .map_err(|_| ErrorCode::QPACK_DECOMPRESSION_FAILED)?;
...
mod tests {
    ...
    async fn rejects_truncation_invalid_huffman_overflow_and_excessive_lengths() {
        for wire in [&[][..], &[0x7f], &[0x7f, 0x80], &[3, b'a', b'b']] {
            assert_eq!(
                be_string_literal(&mut &wire[..], 8).await,
        ...
        let mut input = wire.as_slice();
        assert_eq!(
            be_string_literal(&mut input, 8).await,
    ...
    async fn validates_prefix_and_flags_before_io() {
        for bits in [0, 1, 9, 255] {
            let mut input = &[0, 42][..];
            assert_eq!(
                be_string_literal(&mut input, bits).await,
                Err(ErrorCode::QPACK_ENCODER_STREAM_ERROR)
            );
