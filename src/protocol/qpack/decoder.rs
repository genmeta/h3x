...
mod tests {
    ...
    impl Decoder {
        pub(in crate::protocol::qpack) fn with_channel(
            ...
            let mut decoder = Self::new(local, max_blocked_bytes, max_fields)?;
    ...
    impl Decoder {
        pub(in crate::protocol::qpack) fn poll_feedback(
        ...
        pub(in crate::protocol::qpack) fn read_prefix<'a>(
            &self,
            payload: &'a [u8],
        ) -> Result<(&'a [u8], FieldSectionPrefix)> {
            self.state.read_prefix(payload)
        }
        pub(in crate::protocol::qpack) fn poll_decode(
            &mut self,
            id: u64,
            prefix: FieldSectionPrefix,
            bytes: &[u8],
            ...
            self.state.poll_decode(id, prefix, bytes, cx)
        }
    }
}
