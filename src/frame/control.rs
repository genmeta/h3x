...
use crate::{
    Error, ErrorCode, Result, TransportError,
...
pub(crate) async fn be_stream_type<R: AsyncRead + StopSending + TransportError + Unpin + ?Sized>(
    recv: &mut R,
    ...
    match frame::be_varint(recv).await {
        Ok(Some(ty)) => match StreamType::try_from(ty) {
            Ok(ty) => Ok(Some(ty)),
            Err(_) => {
                recv.stop(ErrorCode::StreamCreationError.as_u64());
                Ok(None)
            }
        },
        Ok(None) => Ok(None),
        Err(error) => match R::map_error(error) {
            Error::Stream(_) => Ok(None),
            error @ Error::Connection(_) => Err(error),
        },
    }
}
...
pub(crate) async fn be_control<R: AsyncRead + Unpin>(recv: &mut R) -> Result<Control> {
    ...
    recv.read_exact(&mut payload)
        ...
        .map_err(|error| crate::Error::from_io(error, ErrorCode::ClosedCriticalStream))?;
    Ok(
        match frame::be_frame_payload(&mut payload.as_slice(), known, length)
            .await
            .map_err(|error| Error::from_io(error, ErrorCode::InternalError).connection())?
        {
